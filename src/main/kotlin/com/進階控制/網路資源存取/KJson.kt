...
fun main() {
    ...
    for (jo in records) {
        jo as JSONObject
        if (jo.get("surplusSpace").toString().toInt() > 500) {
            println("${jo.get("parkName")} ${jo.get("surplusSpace")} / ${jo.get("totalSpace")}")
        }
    }
    println("----------------------------------------------------------------------------")
    // 巨匠桃認 24.990833212079906, 121.31204504349006
    // 請求出最近 1 公里內的停車場停車資訊
    val endLat = 24.990833212079906
    val endLng = 121.31204504349006
    for (jo in records) {
        jo as JSONObject
        val wgsX = jo.getDouble("wgsX")
        val wgsY = jo.getDouble("wgsY")
        val d = distance(endLng, endLat, wgsX, wgsY )
        if (d <= 1000) {
            println("${jo.get("parkName")} $d m/ ${jo.get("surplusSpace")} / ${jo.get("totalSpace")}")
        }
    }
